...
(library
 ...
 (define (new directed order)
   (define (make-row i)
     (if directed
         ...
         (make-vector i +inf.0))) ; CHANGED from original weighted-graph ADT: i instead of i - 1
 ...
 ;; Changed from original weighted-graph ADT: no more condition to check if from = to
