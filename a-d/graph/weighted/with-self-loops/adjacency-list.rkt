...
(library
   ...
   (define (insert-sorted edge prev next! next)
     (cond 
       ((or (null? next)
        ...
        #t)
       ((= (graph-edge-to edge) (graph-edge-to (car next))) ; This has implications for self-loops in practice: they are only stored once (which is logical)
        #f)
       (else
 ...
 (define (delete-edge! graph from to)
   ...
   (define (delete-sorted to prev next! next)
     (cond 
       ((or (null? next)
            (> to (graph-edge-to (car next)))) ; Normally you'd only end up here if you would delete an non-existing edge
        #f)                                    ; With a self-loop in the undirected case, you'll also end up here the second time delete-sorted is called
       ((= to (graph-edge-to (car next)))      ; This is however fine.
        (next! prev (cdr next))
        #t)
       (else
 ...
 ;; Changed from original weighted-graph ADT: no more return of 0 weight if from = to
